...
function App() {
  ...
  const { theme, setAppTheme } = useContext(PostsContext);
      ...
      await feedService.getPosts(page).then(async (response) => {
        ...
        response.data.data.map((post) => {
          addPostInDb({
            ...
            likeCount: post.likes?.data.length || 0,
  ...
  return (
    <div className={theme}>
      <FormGroup sx={{ position: "absolute", right: 5, top: 5 }}>
        <FormControlLabel
          control={
            <MaterialUISwitch
              sx={{ m: 1 }}
              defaultChecked={theme === "dark-theme"}
              onChange={handleThemeChange} // Attach the handler here
            />
          }
          label="Theme"
        />
